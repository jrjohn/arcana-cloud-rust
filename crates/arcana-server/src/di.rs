...
/// Shaku module definitions.
///
/// Kept in their own private module only so the lint allow below can be scoped to
/// the structs that `module!` generates (attributes on a macro invocation are
/// ignored by rustc). Everything is re-exported, so public paths are unchanged.
#[allow(
    clippy::struct_field_names,
    reason = "struct fields are generated by shaku's `module!` macro, which names every one `__di_component_*`"
)]
mod shaku_modules {
    use super::{
        module, AuthServiceComponent, DatabasePool, MySqlUserDaoImpl, PasswordHasher,
        RedisCacheService, RemoteUserRepository, SecurityConfig, TokenProvider,
        UserRepositoryImpl, UserServiceComponent,
    };
...
pub use shaku_modules::{DistributedServiceModule, MonolithicModule, RepositoryModule};
...
/// # Errors
///
/// Propagates the error from [`DatabasePool::connect`] if the database pool
/// cannot be opened, health-checked or migrated.
/// Returns [`arcana_core::ArcanaError::Cache`] if Redis is enabled and the
/// connection pool cannot be created from `redis_config.url`.
...
/// # Errors
///
/// Propagates the error from [`RemoteUserRepository::connect`] if the gRPC
/// channel to `repository_url` cannot be established.
/// Returns [`arcana_core::ArcanaError::Cache`] if Redis is enabled and the
/// connection pool cannot be created from `redis_config.url`.
